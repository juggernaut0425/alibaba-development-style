...
12. 【推荐】在并发场景下，通过双重检查锁\(double-checked locking\)实现延迟初始化的优化问题隐患\(可参考The"Double-Checked Locking is Broken"Declaration\)，推荐问 题解决方案中较为简单一种\(适用于JDK5及以上版本\)，将目标属性声明为volatile型。反例：
